...
export async function PATCH(
  ...
  try {
    ...
    const { name, isActive, currentSongId } = body

    if (!name && isActive === undefined && currentSongId === undefined) {
      return NextResponse.json(
        { error: 'At least one field (name, isActive, or currentSongId) is required' },
        { status: 400 }
      )
    }
