...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const fetchRoom = async () => {
      try {
        ...
        // Set currentSong from room's currentSongId
        if (data.currentSongId) {
          const currentSongItem = data.queueItems?.find((item: QueueItem) => item.youtubeId === data.currentSongId)
          if (currentSongItem) {
            setCurrentSong(currentSongItem)
          }
        }
        setError(null)
      } catch (error) {
    ...
    if (room?.id) {
      ...
      // Subscribe to Room changes (for currentSongId)
      roomSub = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            setRoom(updatedRoom)
            // Update currentSong based on currentSongId
            if (updatedRoom.currentSongId) {
              const currentSongItem = queue.find((item: QueueItem) => item.youtubeId === updatedRoom.currentSongId)
              if (currentSongItem) {
                setCurrentSong(currentSongItem)
              }
            } else {
  ...
  }, [params.code, nickname, router, room?.id, queue])
  ...
  const handlePlayNext = async () => {
    if (queue.length > 0 && room) {
      const nextSong = queue[0]

      // Update the Room's currentSongId in Supabase
      try {
        await fetch(`/api/rooms/${room.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ currentSongId: nextSong.youtubeId }),
        })
      } catch (error) {
        console.error('Error updating currentSongId:', error)
      }
              ...
              {currentSong ? (
                <div className="space-y-4 flex-1 flex flex-col min-h-0">
                  {isHost ? (
                    ...
                    <div className="flex-1 rounded-2xl overflow-hidden border border-purple-500/20 shadow-2xl min-h-0 bg-[#0a0a0a] flex items-center justify-center">
                      <div className="text-center p-8">
                        ...
                        <p className="text-purple-400 text-lg mb-4">Now playing on host device</p>
